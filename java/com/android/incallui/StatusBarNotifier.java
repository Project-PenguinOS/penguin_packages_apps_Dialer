...
public class StatusBarNotifier
  ...
  private void buildAndSendNotification(
    ...
    // addPersonReference(builder, contactInfo, call); // Removed as CallStyle handles Person
  ...
  private void createIncomingCallNotification(
    ...
    if (state == DialerCallState.ACTIVE
      ...
      // Actions are handled by Notification.CallStyle
      // addDismissAction(builder); // Removed
      if (call.isVideoCall()) {
        addVideoCallAction(builder); // Keep this if it's a video call specific action not covered by CallStyle
      } else {
        // addAnswerAction(builder); // Removed
      }
    }
  }
